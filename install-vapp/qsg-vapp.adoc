...
. Nella directory in cui è stato scaricato `.tar` immettere il `tar -xvzf ActiveIQUnifiedManager-<version>.tar.gz` comando.
  +
Il necessario `OVA` file, un certificato root e un `README` i file vengono decompressi nella directory di destinazione.
